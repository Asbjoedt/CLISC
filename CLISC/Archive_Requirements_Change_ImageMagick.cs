...
namespace CLISC
{
    public partial class Archive_Requirements
    {
            ...
            using (SpreadsheetDocument spreadsheet = SpreadsheetDocument.Open(filepath, true))
            {
                ...
                foreach (WorksheetPart worksheetPart in worksheetParts)
                {
                    ...
                    foreach (EmbeddedPackagePart part in packages)
                    {
                        ...
                        //Convert_EmbedPackage(filepath, part);
                    ...
                    foreach (OpenXmlPart part in emf)
                    {
                    ...
                    foreach (OpenXmlPart part in images)
                    {
                        // Create new image in TIFF file format and change relationships
                        Convert_EmbedImg(filepath, worksheetPart, part);
        ...
        public void Convert_EmbedImg(string filepath, WorksheetPart worksheetPart, OpenXmlPart part)
        {
            // Get data
            Uri new_Uri = Get_New_Uri(part.Uri, ".tiff");
            string new_Filename = Get_New_Filename(new_Uri);
            string parentPartType = Get_ParentPart_String(part);
            ...
            ImagePart new_ImagePart = Create_ImagePart(worksheetPart, parentPartType, new_Stream);
        ...
        public Uri Get_New_Uri(Uri part_Uri, string new_Extension)
        {
        ...
        public ImagePart Create_ImagePart(WorksheetPart worksheetPart, string parentPartType, Stream stream)
        {
            ...
            switch (parentPartType)
            {
                ...
                case "DocumentFormat.OpenXml.Packaging.DrawingsPart":
                    ...
                    relationshipId = Get_New_RelationshipId(worksheetPart.DrawingsPart.Parts);
                ...
                // If Excel-generated .emf image
                case "DocumentFormat.OpenXml.Packaging.VmlDrawingPart":
                    // Get new relationship id
                    relationshipId = Get_New_RelationshipId(worksheetPart.Parts);

                    // Add new ImagePart
                    new_ImagePart = worksheetPart.AddImagePart(ImagePartType.Tiff, relationshipId);
                ...
                // If 3D object
                case "DocumentFormat.OpenXml.Packaging.Model3DReferenceRelationshipPart":
                    // Do nothing
                    break;
        ...
        public string Get_New_RelationshipId(IEnumerable<IdPartPair> Parts)
        {
            int number = 1;
            foreach (IdPartPair part in Parts)
            {
                number++;
            }
        ...
        public string Get_ParentPart_String(OpenXmlPart part)
        {
            ...
            foreach (OpenXmlPart parentPart in parentParts)
            {
                ...
                else if (parentPart.ToString() == "DocumentFormat.OpenXml.Packaging.PackagePart")
                {
                    parentPart_String = parentPart.ToString();
                    return parentPart_String;
                }
